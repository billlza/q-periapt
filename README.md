...
| Signatures | ML-DSA-65/87, SLH-DSA | ✅ ML-DSA-65 **and ML-DSA-87** (libcrux) wired & tested (NIST ACVP + RustCrypto differential each); ML-DSA-87 is the enhanced-mode (L5) signature; SLH-DSA-SHA2-128s/256s (fips205) behind the off-by-default `slh-dsa` feature |
...
| KATs / differential tests | X-Wing draft + FIPS 203 ACVP vectors, multi-backend differential | 🟡 byte-exact **X-Wing draft KAT PASSES** (3 official `draft-connolly-cfrg-xwing-kem` vectors); **multi-backend differential PASSES** over the whole KEM chain (`src/differential.rs`) — ML-KEM-768 **and ML-KEM-1024** vs RustCrypto `ml-kem`, X25519 vs `orion` + RFC 7748, and the full `HybridKem` reconstructed from independent ML-KEM + X25519 + SHA3; and **ML-DSA-65 and ML-DSA-87 vs RustCrypto `ml-dsa`** (byte-identical keygen + signatures, cross-verification both directions, tamper rejection) — all byte-identical on random inputs; **NIST ACVP** ground-truth conformance PASSES (`src/acvp.rs`) — the full ML-KEM-768 **and ML-KEM-1024** sets (60 cases each, incl. implicit-rejection) + **ML-DSA-65 and ML-DSA-87** keygen/sig **across signature modes** — external/pure (deterministic + **hedged**, with **non-empty contexts**) and **HashML-DSA SHAKE-128 pre-hash** (`acvp_ml_dsa_*_signature_modes`); the internal interface / `externalMu` / non-SHAKE128 pre-hash are not exposed by libcrux (documented), and ML-DSA-44 / ML-KEM-512 + an SLH-DSA interop differential remain pending; **property-based tests** (proptest, `src/proptests.rs`) hold the combiner + hybrid invariants — binding injectivity, determinism, domain separation, the guards, and KEM round-trip — over random inputs |
