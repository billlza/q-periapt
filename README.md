...
> crates.io publication pending / doctoral-thesis project (v0.1.2, ABI 2).**
> The formal stable GitHub transactions use the Apple XCFramework tag `v0.1.2`
> and the `abi2-platforms-v0.1.2` platform tag (Android AAR and GNU/Linux
...
> “stable” label does not claim Rust API 1.0 compatibility: SemVer `0.1.2` remains
> a Rust `0.x` line, while C ABI 2 and the ten exact internal `=0.1.2` dependency
...
`0.1.2` stable-version ABI 2 source boundary deliberately exposes only `ContextBound`: its nine-symbol C
...
| Dimension | Target | Today (v0.1.2 stable-version ABI 2 source line) |
|---|---|---|
...
`v0.1.1`, `abi2-platforms-v0.1.0`, and `abi2-platforms-v0.1.1` tags are permanent
under the no-bypass tag ruleset but were never published; the correction ships as
0.1.2 because stable tags are immutable and publication commits may change only
`artifact/results.json` (see the 0.1.0 and 0.1.1 history in
[`artifact/stable-release-notes.md`](artifact/stable-release-notes.md)).

| Tag | Scope | Toolchain | Status |
|---|---|---|---|
| [`abi2-platforms-v0.1.2`](https://github.com/billlza/q-periapt/releases/tag/abi2-platforms-v0.1.2) | Android AAR (arm64-v8a/armeabi-v7a/x86/x86_64, 16 KiB pages, API 35 emulator runtime evidence), GNU/Linux x86_64+aarch64 SDK tars | Rust 1.96.1 | Stable target; current only with a verified receipt |
| [`v0.1.2`](https://github.com/billlza/q-periapt/releases/tag/v0.1.2) | Apple Developer ID-signed static XCFramework ZIP | Rust 1.96.1 | Stable target; current only with a verified receipt |
...
current (the stable target is `v0.1.2`). The asset is binaryTarget material and does not turn `bindings/swift/Package.swift` into a
...
`abi2-platforms-v0.1.2` transaction; `artifact/results.json` separately
...
- [`artifact/stable-release-notes.md`](artifact/stable-release-notes.md) — the ABI 2 `0.1.2` Apple/platform transaction, hardening scope, verification commands, and explicit non-goals ✅
