...
| FFI / bindings | C ABI + Swift + Kotlin/JVM + Android AAR/JNI + WASM | 🟡 the unpublished **ABI 2** native product surface is reduced to nine exact C exports: metadata (5), signed-policy decision, OS-CSPRNG atomic key generation, OS-CSPRNG encapsulation, and decapsulation. The current macOS C archive and clean-tree, source-bound single-iPad diagnostic pass; the five-slice Apple XCFramework, four-ABI Android AAR, and schema-2 diagnostic release index pass isolated-consumer and semantic validation on their recorded sources. Android JNI consumes rather than duplicates the contract. Deterministic/X-Wing/combine checks remain internal Rust/WASM conformance evidence rather than a native product bypass. Kotlin still requires JDK 22+; Android ART, a clean same-source iPad+iPhone matrix, Linux SONAME execution, and Windows PE packaging remain separate gates. Removing the PQClean graph invalidated every pre-change Apple/performance proof; the single-iPad diagnostic has been regenerated, while the paired matrix and matched-performance proof remain stale. Release provenance is additionally blocked by the unsuppressed upstream `proc-macro-error2` maintenance advisory inherited through libcrux/hax, stale release-matrix/performance evidence, cross-platform runtime evidence, and public signing/attestation. |
