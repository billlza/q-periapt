...
| KATs / differential tests | X-Wing draft + FIPS 203 ACVP vectors, multi-backend differential | 🟡 byte-exact **X-Wing draft KAT PASSES** (3 official `draft-connolly-cfrg-xwing-kem` vectors); **multi-backend differential PASSES** over the whole KEM chain (`src/differential.rs`) — ML-KEM-768 **and ML-KEM-1024** vs RustCrypto `ml-kem`, X25519 vs `orion` + RFC 7748, and the full `HybridKem` reconstructed from independent ML-KEM + X25519 + SHA3; and **ML-DSA-65 vs RustCrypto `ml-dsa`** (byte-identical keygen + signatures, cross-verification) — all byte-identical on random inputs; **NIST ACVP** ground-truth conformance PASSES (`src/acvp.rs`) — the full ML-KEM-768 **and ML-KEM-1024** sets (60 cases each, incl. implicit-rejection) + ML-DSA-65 keygen/sig; broader ACVP signature-mode coverage (ML-DSA-44/87) and an SLH-DSA interop differential still pending; **property-based tests** (proptest, `src/proptests.rs`) hold the combiner + hybrid invariants — binding injectivity, determinism, domain separation, the guards, and KEM round-trip — over random inputs |
