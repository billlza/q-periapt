...
> NOT FIPS-validated — do not use in production yet.** Pinned third-party
> implementations are wired for ML-KEM via `fips203` 0.4.3,
...
- **Side-channel CI as an assurance feature** — the failure-path indistinguishability
  ...
  self-validating `fips203` ML-KEM decapsulation gap probe with the corrected
  ŝ `[0..1152]` + z `[2368..2400]` marking and requires zero reports. This
  probe was migrated with the backend, so prior `libcrux` result logs are historical
  and a fresh two-ISA run is required for the release source digest. Other
...
- **Primitive performance** — it wires `fips203` (ML-KEM), `fips204` (ML-DSA),
  RustCrypto `sha3`,
...
| FFI / bindings | C ABI + Swift + Kotlin/JVM + Android AAR/JNI + WASM | 🟡 the unpublished **ABI 2** native product surface is reduced to nine exact C exports: metadata (5), signed-policy decision, OS-CSPRNG atomic key generation, OS-CSPRNG encapsulation, and decapsulation. Previously recorded macOS C, Swift XCFramework, Android AAR, Apple-device, and matched-performance results are historical after the backend/source-digest migration; each package and runtime/performance proof must be regenerated and live-verified against the release source. Android JNI consumes rather than duplicates the contract. Deterministic/X-Wing/combine checks remain internal Rust/WASM conformance evidence rather than a native product bypass. The former `proc-macro-error2` advisory path was removed with `libcrux`/hax and `cargo audit --deny warnings` is now clean, without an ignore. Release is still blocked by incomplete same-source cross-platform/runtime evidence, independent cryptographic and ABI review, clean signed provenance, and public distribution signing/attestation. |
...
│   ├── q-periapt-backends  # ✅ publishable third-party backends (fips203/fips204/sha3, x25519-dalek, optional fips205)
...
- **What is real now** (each grounded in committed code — read it before relying on
  ...
  - Real third-party backends are wired in `q-periapt-backends`: ML-KEM-512/768/1024
    via `fips203` 0.4.3, ML-DSA-44/65/87 via `fips204` 0.4.6, SHA3/SHAKE via
...
- **Still scaffolded / pending** (do not assume these are finished): recapturing the
  migrated `fips203` binary-CT cells on x86_64+aarch64, extending binary-level CT to
  ...
  to the production backend. **Done since earlier drafts** (no longer pending): the
...
- The wired backends are pre-1.0 / unaudited for this integration to varying degrees.
  Versions are pinned and kept swappable for differential testing and CVE mitigation.
  A clean dependency-advisory scan is necessary but is not an independent cryptographic,
  side-channel, or ABI audit.
