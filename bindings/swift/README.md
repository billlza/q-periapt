...
```sh
QPERIAPT_APPLE_RELEASE_CONFIRM=v0.1.3 \
...
unexpected mode is rejected. `APPLE_DISTRIBUTION.json` binds the `v0.1.3` release
...
The stable Apple target is `v0.1.3` (Rust 1.96.1), requires
...
target the separate `abi2-platforms-v0.1.3` transaction; unsigned Windows output
