...
MSVC** (links `q_periapt_ffi.dll`) and **macOS / aarch64 / clang** (links the dylib): all three
checks pass, with byte-identical `combine()` output (Test A) on both. CI is configured to run
this test — the `windows` job runs `build-and-run.bat`, the `check` job runs `build-and-run.sh`
on Linux — but this repo has **no git remote**, so CI has not executed; both runs above were by
hand. (Broad cross-ISA assurance comes from the Rust `cross` job, not this C test.)
