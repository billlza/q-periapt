...
> ABI 2 / `0.1.2` is the stable-version Rust crate line and is pre-publication package-ready;
...
> GitHub stable targets are the Apple XCFramework `v0.1.2` (Rust 1.96.1;
> the earlier `v0.1.0-alpha.2` on 1.96.0 is superseded and the `v0.1.0` and
> `v0.1.1` tags were never published) and the
> `abi2-platforms-v0.1.2` platform distribution (Android AAR + API 35 / 16 KiB-page
...
targets are the Apple `v0.1.2` XCFramework and the
`abi2-platforms-v0.1.2` Android/Linux packages; publication/currentness is
