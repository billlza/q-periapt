...
version `0.1.4`. Its `cdylib`/DLL exposes exactly nine dynamic
...
This is the **`0.1.4` stable-version ABI 2
source/crate contract**, the successor of the published `0.1.3` release line; the
`0.1.4` crate set is pre-publication package-ready, and its
no-upload package checks do not establish registry publication of the `0.1.4` crates.
