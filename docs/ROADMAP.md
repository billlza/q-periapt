...
ABI 2 / `0.1.4` is the stable-version source line, succeeding the published
`0.1.3` release, with `0.1.4` registry publication
...
`v0.1.4` and the `abi2-platforms-v0.1.4` platform
...
5. **Embedding and package distribution.** `artifact/embedding-readiness.sh` now gives
   ...
   XCFramework, `v0.1.4`), Android (four-ABI AAR + emulator runtime evidence),
   and GNU/Linux (x86_64+aarch64 SDK tars)
   through the `abi2-platforms-v0.1.4` transaction once its immutable-public
   ...
   Package `0.1.4` is the stable-version source/crate line and has a
   ...
   stable binary targets are the independently evidence-selected Apple `v0.1.4`
   XCFramework and `abi2-platforms-v0.1.4` Android/Linux packages,
...
| C ABI 2 stable package readiness | **The 0.1.4 source/crate contract is stable-version and pre-publication package-ready; crates.io upload-API acceptance, crate-name ownership, publishing credentials/authorization, server-side policy acceptance, and a verified registry receipt remain separate gates. The Apple `v0.1.4` XCFramework and stable Android/Linux packages are the coordinated GitHub targets; attested public status requires verified receipts and `prerelease=false`. Same-source device/performance evidence, signed or transparency-backed source provenance, and independent cryptographic/C-FFI/ABI audit remain required for production promotion; ART-rerun currentness is tracked live in `artifact/results.json`.** |
| Stable immutable GitHub targets (`v0.1.4` Apple receipt + `abi2-platforms-v0.1.4` receipt) | **Stable targets remain governed by versioned receipts under `results.json.release_publications`; historical alpha.2 receipts remain immutable. A receipt does not promote unrelated device/performance evidence. The legacy `swift_xcframework.distribution` field is only an exact active projection. Stable publication is not a production, registry, or store-readiness claim.** |
| liboqs-style package distribution surface (crates/C archive/XCFramework/AAR) | Partial; historical Apple XCFramework + Android AAR + Linux/Windows C SDK GitHub prereleases are bound to their exact receipts, and the ten `0.1.3` crates are published on crates.io, while current target-selected rebuilds, a complete remote Swift package, and `0.1.4` crates.io plus Maven/deb/rpm/MSIX registry publication remain pending |
