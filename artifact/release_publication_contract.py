...
def _is_legacy_alpha2_selector(
    ...
    return (
        ...
        and apple_contract.APPLE_V0_1_0_PUBLICATION_KEY not in publications
...
def _stable_cohort_state(publications: dict[str, object]) -> str:
    apple = publications.get(apple_contract.APPLE_V0_1_0_PUBLICATION_KEY)
    platform = publications.get(
        stable_platform_contract.PLATFORM_V0_1_0_PUBLICATION_KEY
    )
    ...
    if apple is not None and platform is not None and crates is None:
        ...
        if (
            ...
            == stable_platform_contract.PLATFORM_V0_1_0_STATUS_PENDING
        ):
    ...
    if apple is not None and platform is not None and crates is not None:
        ...
        if (
            ...
            and platform_receipt.get("status")
            == stable_platform_contract.PLATFORM_V0_1_0_STATUS_VERIFIED
...
def _validate_source_crosslinks(
    ...
    domain_sources = [
        _source_identity(
            _source_object(
                publications[apple_contract.APPLE_V0_1_0_PUBLICATION_KEY],
                domain="Apple",
            ),
            "Apple stable",
        ),
