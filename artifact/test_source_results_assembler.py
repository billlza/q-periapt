...
class SourceResultsAssemblerTests(unittest.TestCase):
    ...
    def test_initial_and_installed_proof_maps_are_exactly_distinct(self) -> None:
        ...
        self.assertEqual(226, len(assembler.PROOF_TO_BYTE_INPUT_PATHS))
        self.assertEqual(36, len(assembler.INITIAL_BASELINE_MISSING_PROOF_INPUT_KEYS))
        self.assertEqual(190, len(initial["proof_to_byte_inputs"]))
        self.assertEqual(226, len(installed["proof_to_byte_inputs"]))
        self.assertEqual(
    ...
    def test_rust_handoff_resample_is_mandatory_and_byte_exact(self) -> None:
        ...
        crate_file = assembler.FileSnapshot(
            path=manifest.path.parent / "q-periapt-core-0.1.0.crate",
            data=b"crate\n",
            size=6,
            sha256="8" * 64,
        )
