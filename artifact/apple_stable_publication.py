#!/usr/bin/env python3
"""Assemble and promote the exact Apple 0.1.4 stable publication receipt.
...
APPLE_PUBLIC_DISTRIBUTION_NAME = "q-periapt-swift-0.1.4"
...
APPLE_PUBLICATION_RECEIPT_NAME = "apple-v0.1.4-publication-receipt.json"
...
REMOTE_CONSUMER_BOUNDARY = (
    "Atomic evidence commit for one fresh Apple 0.1.4 stable URL binary consumer "
...
class AppleStablePublicationError(ValueError):
    """Apple 0.1.4 stable receipt evidence or state transition is invalid."""
...
def _validate_clean_annotated_tag(
    ...
    try:
        ...
        raise AppleStablePublicationError(
            "cannot establish the Apple 0.1.4 stable source/tag boundary"
        ) from exc
    ...
    _require(
        ...
        "Apple 0.1.4 stable release tag must be one annotated tag object",
    )
    _require(
    ...
    try:
        manifest = {
            "release_publications": {
                apple_contract.APPLE_V0_1_4_PUBLICATION_KEY: pending_receipt
            }
        }
...
def _validate_remote_receipt(value: object) -> dict[str, Any]:
    ...
    _require(
        ...
        == apple_contract.APPLE_V0_1_4_IDENTITY,
...
def _pending_leaf_from_results(
    ...
    pending = _object(
        publications.get(apple_contract.APPLE_V0_1_4_PUBLICATION_KEY),
        "current Apple 0.1.4 stable publication receipt",
    )
    _require(
        pending.get("status") == apple_contract.APPLE_STATUS_PENDING,
        "Apple promotion requires a current pending 0.1.4 stable receipt",
    )
    return pending
...
def promote_receipt(
    ...
    distribution = _object(
        pending["distribution"], "pending Apple 0.1.4 stable distribution"
    )
    ...
    verified_distribution = _object(
        verified["distribution"], "verified Apple 0.1.4 stable distribution"
    )
    ...
    try:
        ...
        apple_contract.validate_apple_publication_transition(
            {
                "release_publications": {
                    apple_contract.APPLE_V0_1_4_PUBLICATION_KEY: pending
                }
            },
            {
                "release_publications": {
...
def _publish_receipt(receipt: object) -> tuple[pathlib.Path, str]:
    return create_private_transaction_json(
        ...
        value=receipt,
        label="Apple 0.1.4 stable publication receipt",
        maximum=MAX_REMOTE_RECEIPT_BYTES,
    )
...
def _success_marker(path: pathlib.Path, digest: str, status: str) -> str:
    ...
    return (
        "APPLE_V0_1_4_PUBLICATION_RECEIPT_PASS "
