#!/usr/bin/env python3
"""Collect the frozen 0.1.5 stable platform publication transaction.
...
from platform_stable_publication_contract import (
    ...
    PlatformV015PublicationContractError,
...
FRESH_RECORD_KIND = "qperiapt.platform_v0_1_5_fresh_download_verification"
...
class PlatformV015PublicationError(ValueError):
    """The platform publication transaction violates a local or remote gate."""


class PlatformV015PublicationRetryableError(PlatformV015PublicationError):
...
def _fail(message: str) -> Never:
    raise PlatformV015PublicationError(message)


def _retryable(reason: str) -> Never:
    raise PlatformV015PublicationRetryableError(f"retryable:{reason}")
...
def _normalize_direct_child(
    ...
    *,
    ...
    try:
        ...
        raise PlatformV015PublicationError(f"cannot inspect {label}") from exc
...
def _raise_github_execution_error(
    ...
    *,
    ...
    raise PlatformV015PublicationError(str(error)) from error
...
def _capture_command(
    ...
    *,
    ...
    try:
        ...
        raise PlatformV015PublicationError(f"{label} failed safely") from exc
...
def _git_line(
    ...
    *,
    ...
    try:
        ...
        raise PlatformV015PublicationError(f"{label} is not ASCII") from exc
...
def assemble_pending_receipt(
    ...
    *,
    ...
    receipt: dict[str, object] = {
        ...
        "schema_version": PLATFORM_V0_1_5_PUBLICATION_SCHEMA_VERSION,
        "status": PLATFORM_V0_1_5_STATUS_PENDING,
    }
    try:
...
def main(argv: Sequence[str]) -> int:
    ...
    try:
        ...
        return 125
    except PlatformV015PublicationRetryableError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    except (OSError, PlatformV015PublicationError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
    return 0
