#!/bin/sh
# Re-download and independently verify the immutable 0.1.4 Apple release set.
set -eu
umask 077
...
if [ "$URL" != "$ZIP_URL" ]; then
	printf 'error: remote consumer URL must equal the immutable 0.1.4 release asset URL\n' >&2
	exit 2
fi
require_lower_hex() {
	value=$1
