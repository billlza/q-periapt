...
def pending_receipt() -> dict[str, object]:
    ...
    return {
        "boundary": contract.PLATFORM_V0_1_5_PUBLICATION_BOUNDARY,
        "identity": {
        ...
        "kind": contract.PLATFORM_V0_1_5_PUBLICATION_KIND,
        "observation": {
        ...
        "schema_version": contract.PLATFORM_V0_1_5_PUBLICATION_SCHEMA_VERSION,
        "status": contract.PLATFORM_V0_1_5_STATUS_PENDING,
    }


def verified_receipt() -> dict[str, object]:
    receipt = pending_receipt()
    receipt["status"] = contract.PLATFORM_V0_1_5_STATUS_VERIFIED
...
class PlatformV015PublicationContractTests(unittest.TestCase):
    def validate(self, receipt: object) -> None:
        contract.validate_v0_1_5_publication_receipt(receipt)
        ...
        for value in (
            ...
            with self.subTest(value=value):
                with self.assertRaises(
                    contract.PlatformV015PublicationContractError
                ):
    ...
    def test_v0_1_5_identity_and_current_proof_schema_authority_are_explicit(
        self,
    ) -> None:
        # The active platform identity deliberately derives from the
        # current candidate contract, so the version sweep that bumps
        # platform_distribution_contract to 0.1.5 needs no change here.
        self.assertEqual(
        ...
        self.assertEqual(
            contract.PLATFORM_V0_1_5_PUBLICATION_KEY,
            "platform_v0_1_5",
        )
        self.assertNotIn(
            "zero-result",
            contract.PLATFORM_V0_1_5_PUBLICATION_BOUNDARY,
        )
        self.assertIn(
        ...
        self.assertEqual(contract.PLATFORM_V0_1_5_PUBLICATION_SCHEMA_VERSION, 3)
        self.assertEqual(
