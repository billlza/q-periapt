...
\code{tc netem}. Beyond attaining the ceiling, we prove a machine-checked
\emph{shape\,$\times$\,format separation}: in one EasyCrypt model, a lean (X-Wing-shaped) combiner
that omits the PQ public key \emph{loses} \mbox{MAL-BIND-K-PK} over the FIPS-203 expanded
decapsulation-key serialization---a concrete adversary wins with probability one---yet
\emph{regains} it over the seed-\code{dk} serialization, while ContextBound, which absorbs the
public key, is binding over \emph{both}; the asymmetry (the full shape reduces to
collision-resistance \emph{alone}, the seed-\code{dk} lean shape additionally needs injectivity of
the rejection function) is the formal case for the construction. We are explicit about what is and
...
\emph{shape\,$\times$\,format separation} (\cref{thm:sep}): the lean (X-Wing-shaped) combiner
\emph{loses} \mbox{MAL-BIND-K-PK} over the expanded-\code{dk} serialization---a concrete adversary
wins with probability one---but \emph{regains} it over seed-\code{dk}, while ContextBound is binding
over \emph{both}, isolating public-key absorption as the determining factor. This is a stated,
proven result, not an integration claim.
...
\subsection{A mechanized shape\,$\times$\,format separation}\label{sec:sep}
The headline of \cref{sec:kernel} is that ContextBound \emph{attains} the binding ceiling. Here we
prove a complementary result that is the paper's distinctive cryptographic contribution: a
\emph{machine-checked separation} pinning down exactly \emph{which combiner shapes attain
\mbox{MAL-BIND-K-PK} over which component \code{dk} serializations}. The separation is established in
\emph{one} EasyCrypt model---a single execution type, a single shared-secret wiring $ss=J(z,ct)$,
the two combiners differing \emph{only} in whether the PQ public key is absorbed---so any difference
in outcome is attributable to the combiner \emph{shape}, not to a difference in the modeled KEM.

\begin{theorem}[shape\,$\times$\,format separation, machine-checked]\label{thm:sep}
Model the implicit-rejection secret as $ss=J(z,ct)$ (FIPS-203 \S6.3). Then, in a single binding
model:
\begin{enumerate}
\item \emph{(expanded-\code{dk}, $z$ adversary-substitutable.)} The \textbf{lean} combiner that omits
$pk_{\mathrm{pq}}$ is \emph{not} \mbox{MAL-BIND-K-PK}: a concrete, assumption-free, deterministic
adversary wins with probability~$1$ (\code{lean\_kpk\_broken}). The \textbf{full} combiner that
absorbs $pk_{\mathrm{pq}}$ satisfies $\mathsf{Adv}^{\text{MAL-BIND-K-PK}}\le\mathsf{Adv}^{\mathrm{CR}}(H)$
(\code{full\_kpk\_le\_cr}).
\item \emph{(seed-\code{dk}, $z$ key-derived.)} The \textbf{lean} combiner \emph{regains}
\mbox{MAL-BIND-K-PK}, $\le\mathsf{Adv}^{\mathrm{CR}}(H)$, under the idealization that $J$ and the seed
derivation are injective (\code{lean\_kpk\_seed\_le\_cr})---a strictly \emph{stronger} assumption than
the full shape, which reduces to $\mathrm{CR}(H)$ \emph{alone}. The full combiner is safe a~fortiori.
\end{enumerate}
\end{theorem}

\noindent The break works precisely because, over expanded-\code{dk}, an adversary can equalize the
stored seed $z$ across two otherwise-distinct keypairs, so a garbage ciphertext implicit-rejects to
the \emph{same} $J(z,ct)$ under $\code{ek}_0\neq\code{ek}_1$ (Schmieg~\cite{schmieg}); the lean key
then collides while the public key differs---a genuine \mbox{MAL-BIND-K-PK} violation, not an
artifact of omitting $pk_{\mathrm{pq}}$ (the win vanishes the moment $ss$ depends on the key, which
is exactly what seed-\code{dk} enforces). Each half is load-bearing: removing the two-distinct-keys
hypothesis collapses the break, and removing the encoding injectivity or the $J$/seed injectivity
collapses the seed-\code{dk} safety (verified by deleted-hypothesis negative controls).

We are deliberately precise about scope. (i)~$J(z,ct)$ is \emph{modeled} after the FIPS-203
implicit-rejection formula, not \emph{derived} from a mechanized Decaps; the statement is about the
combiner, and the component shared secrets are otherwise inert. (ii)~The lean break is specific to
the \emph{expanded}-\code{dk} serialization---this is \emph{not} a break of X-Wing, which mandates
seed-\code{dk}. (iii)~The safety bounds are conditional on $H$'s collision resistance. (iv)~Note the
honest \emph{asymmetry}: the full shape reduces to $\mathrm{CR}(H)$ \emph{alone}, whereas the
seed-\code{dk} lean shape additionally needs injectivity of the rejection function and the seed
derivation. That asymmetry \emph{is} the engineering case for ContextBound: absorbing
$pk_{\mathrm{pq}}$ removes a latent dependence on the component library's serialization choice---a
dependence an implementation that caches expanded keys for performance could silently reintroduce.
We complement the proof with an executable regression oracle that exercises the same coupling
against real libcrux as a CI-configured gate.
...
\midrule
\multicolumn{2}{@{}l}{\emph{Separation (\cref{thm:sep}); one model, combiners differ only in $pk_{\mathrm{pq}}$}}\\
\code{lean\_kpk\_broken} & lean shape, expanded-\code{dk}: concrete adversary, $\Pr=1$ \\
\code{full\_kpk\_le\_cr} & full shape, expanded-\code{dk}: $\le\mathrm{CR}(H)$ \\
\code{lean\_kpk\_seed\_le\_cr} & lean shape, seed-\code{dk}: $\le\mathrm{CR}(H)$ (needs $J$/seed inj.) \\
\bottomrule
\end{tabular}
\end{table}
