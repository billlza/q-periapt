...
derivation are injective (\code{lean\_kpk\_seed\_le\_cr}). The full combiner is safe a~fortiori.
\end{enumerate}
\end{theorem}
...
RTT~$\approx$~0 the per-group medians are tight to $\pm 2$--$3\,\mu$s and the ordering is fully
resolved: classical X25519 $238\,\mu$s, the IANA-standard \code{X25519MLKEM768} $263\,\mu$s,
CompatXWing $317\,\mu$s, ContextBound $326\,\mu$s. The reproducible cost of the PQ/T upgrade is thus
$\approx 88\,\mu$s of ML-KEM CPU over the classical baseline, and---critically---\textbf{ContextBound
costs only $+8.6\,\mu$s over CompatXWing}, the price of the stronger hash-everything binding, in
close agreement with the $+6.2\,\mu$s combiner cost measured in isolation (\cref{tab:bench}). This
...
repetitions). At RTT~0 the per-group order resolves to $\pm$few-$\mu$s and ContextBound is
$+8.6\,\mu$s over CompatXWing; at RTT~$\ge$~20\,ms the combiner cost is dwarfed by the link. Raw
...
X25519MLKEM768 (IANA standard)      & $263.4\,\mu$s & $41.15$\,ms & $101.17$\,ms \\
Q-Periapt CompatXWing               & $317.3\,\mu$s & $41.14$\,ms & $101.17$\,ms \\
...
\begin{table}[h]\centering\footnotesize
\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.12}
\caption{Time-to-session under jitter/loss, p50/p99 ($\mu$s). The p99 under loss is RTO-dominated
and statistically indistinguishable across groups.}\label{tab:loss}
\begin{tabular}{@{}l r r r@{}}
\toprule
