...
RTT-dominated, and the PQ/T overhead is a \emph{fixed} cost ($\sim$200\,$\mu$s of ML-KEM CPU plus
the wire bytes of \cref{fig:wire}), so it becomes negligible---within $0.1$--$0.4\%$ of
classical---at realistic RTT. The hybrid's $+2.27$\,KB (one ML-KEM-768 keyshare each way) fits
within the existing TLS flights and triggers no additional round-trip. The two combiner profiles
are indistinguishable: the stronger binding of the hash-everything combiner is free in latency.
We report these measurements honestly as obtained on a virtualized host; the camera-ready figures
should be reproduced on quiesced bare-metal hardware using the provided protocol script (the
RTT-dominated points are already stable).

\begin{figure}[t]\centering
\includegraphics[width=\columnwidth]{fig_netem.pdf}
\caption{Time-to-session under real \code{tc netem}. (a) latency is RTT-dominated---the three
curves coincide. (b) the PQ/T overhead is a $\sim$fixed CPU cost, hence a negligible percentage at
realistic RTT.}
\label{fig:netem}
\end{figure}
