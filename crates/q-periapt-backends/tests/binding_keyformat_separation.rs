...
//! expanded-dk MAL-BIND-K-PK counterexample against the shipped **fips203 ML-KEM-768**, used to
...
//! forbids, but which production APIs — the `fips203` adapter here — consume and may cache/transport),
...
fn lean_xwing_shape_over_expanded_dk_loses_k_pk_contextbound_keeps_it() {
    ...
    eprintln!(
        "combiner dk-format-coupling witness (shipped fips203 ML-KEM-768):\n  \
...
fn seed_dk_control_z_bound_to_key_closes_schmieg_vector() {
    ...
    eprintln!(
        "seed-dk negative control (shipped fips203 ML-KEM-768, z derived from a 32-byte seed):\n  \
