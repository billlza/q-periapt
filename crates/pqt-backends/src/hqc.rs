...
//!   `C2PRI = false` confines HQC to [`pqt_core::Profile::ContextBound`].
    ...
    ($name:ident, $m:ident, $alg:literal, $pk:literal, $sk:literal, $ct:literal, $ss:literal) => {
        ...
        impl Kem for $name {
            ...
            fn encapsulate(
                ...
                let public =
                    pqcrypto_hqc::$m::PublicKey::from_bytes(pk).map_err(|_| Error::InvalidLength)?;
            ...
            fn decapsulate(&self, sk: &[u8], ct: &[u8], ss: &mut [u8]) -> Result<(), Error> {
                let secret =
                    pqcrypto_hqc::$m::SecretKey::from_bytes(sk).map_err(|_| Error::InvalidLength)?;
...
mod tests {
    ...
    fn hqc128_roundtrip() {
        ...
        kem.encapsulate(&pk, &[0u8; 32], &mut ct, &mut ss_e).unwrap();
    ...
    fn hqc128_ignores_injected_randomness() {
        ...
        assert_ne!(c1, c2, "HQC encaps must be non-deterministic (ignores injected randomness)");
    }
    ...
    fn hqc_rejects_bad_length() {
        ...
        assert!(kem.decapsulate(&[0u8; 10], &[0u8; Hqc128::CT_LEN], &mut ss).is_err());
    }
}
