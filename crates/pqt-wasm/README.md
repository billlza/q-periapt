...
cargo build -p pqt-wasm --target wasm32-unknown-unknown

# Full JS package with bindings (needs wasm-pack):
wasm-pack build crates/pqt-wasm --target web
```

## API

```ts
import init, { mlkem768_keypair, x25519_keypair, encapsulate, decapsulate } from "./pkg/pqt_wasm.js";
await init();
...
`bindings/shared-test-vectors.json` (`cargo test -p pqt-wasm`,
